...
fn net_event(ip: &str, port: &str, domain: Option<&str>, observed_at: &str) -> NormalizedTelemetryEvent {
...
fn public_ip_without_domain_emits_rare_and_ip_literal_signals() {
    ...
    let report = analyzer.analyze(net_event("203.0.113.10", "443", None, "2026-06-27T09:00:00Z"));

    assert_eq!(report.stats.observed, 1);
    assert_eq!(report.stats.handled, 1);
    assert_eq!(report.tracked_destinations, 1);
    assert!(report.signals.iter().any(|s| s.name == "rare_external_destination"));
    assert!(report.signals.iter().any(|s| s.name == "ip_literal_connection"));
}
...
fn suspicious_domain_and_high_risk_port_emit_signals() {
    ...
    assert!(report.signals.iter().any(|s| s.name == "suspicious_dns_pattern"));
    let port_signal = report.signals.iter().find(|s| s.name == "high_risk_port").unwrap();
...
fn repeated_equal_intervals_emit_beacon_candidate() {
    let mut analyzer = NetworkAnalyzer::default();
    analyzer.analyze(net_event("198.51.100.7", "443", Some("node.example.net"), "2026-06-27T09:00:00Z"));
    analyzer.analyze(net_event("198.51.100.7", "443", Some("node.example.net"), "2026-06-27T09:01:00Z"));
    let report = analyzer.analyze(net_event("198.51.100.7", "443", Some("node.example.net"), "2026-06-27T09:02:00Z"));
...
fn private_destination_is_not_rare_external() {
    let mut analyzer = NetworkAnalyzer::default();
    let report = analyzer.analyze(net_event("192.168.1.10", "443", None, "2026-06-27T09:00:00Z"));
