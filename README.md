...
## 🚀 Yeni Monolitik Mimari (Tekil EXE) Güncellemesi
Proje mimarisi "Tauri Sidecar" üzerinden iki ayrı process ile iletişim (Named Pipes) kurmak yerine **Monolitik (Tek Parça) Mimari**'ye dönüştürülmüştür. Bu büyük mimari değişiklikle:
1. **Tek EXE:** Arka plan Kernel/ETW okuyucu motoru (Backend) ve Tauri Arayüzü (Frontend) aynı `.exe` dosyası içinde (sentra-ui.exe) birleştirilmiştir. 
2. **Sıfır İletişim Gecikmesi:** IPC (Inter-Process Communication) boruları ve JSON serileştirme yükü kaldırılarak, işletim sisteminden gelen milyonlarca ETW verisi ve Process bilgisi doğrudan (Memory üzerinden) arayüze 0 gecikmeyle aktarılmaktadır.
3. **Güvenlik İzinleri:** Uygulama doğrudan yerleşik UAC (Yönetici) ayrıcalıklarıyla (requireAdministrator manifest) başlatılarak, güvenlik duvarı "Erişim Engellendi (os error 5)" sorunlarının tamamı çözülmüştür.
4. **Bağımsızlık:** EDR, klasik bir Antivirüs programı gibi tek tıklamayla kurulup (`.msi` / `.exe`) dış bağımlılıklara ihtiyaç duymadan çalışabilir hale gelmiştir.
